if not _G.esp_loaded then
    ...
    _G.Chunks = dx9.FindFirstChild(Workspace, "Chunks")
    ...
    local s_rifle = col_ammo_left:section({ name="rifle ammo" })
    s_rifle:toggle({ name="AK47 Ammo 30",    flag="ammo_ak47_30",    default=false })
    s_rifle:toggle({ name="AK47 Ammo 40",    flag="ammo_ak47_40",    default=false })
    s_rifle:toggle({ name="AK Ammo 45",      flag="ammo_ak_45",      default=false })
    s_rifle:toggle({ name="STANAG Ammo 50",  flag="ammo_stanag_50",  default=false })
    s_rifle:toggle({ name="STANAG Ammo 100", flag="ammo_stanag_100", default=false })
    s_rifle:toggle({ name="M14 Ammo 50",     flag="ammo_m14_50",     default=false })
    s_rifle:toggle({ name="AR10 Ammo 30",    flag="ammo_ar10_30",    default=false })
    s_rifle:toggle({ name="M3 Ammo 30",      flag="ammo_m3_30",      default=false })

    local s_lmg = col_ammo_left:section({ name="lmg ammo" })
    s_lmg:toggle({ name="MK48 Ammo 100", flag="ammo_mk48_100", default=false })
    s_lmg:toggle({ name="M249 Ammo 100", flag="ammo_m249_100", default=false })
    s_lmg:toggle({ name="PKP Ammo 200",  flag="ammo_pkp_200",  default=false })

    local s_pistol = col_ammo_right:section({ name="pistol / smg ammo" })
    s_pistol:toggle({ name="TEC9 Ammo 32",    flag="ammo_tec9_32",  default=false })
    s_pistol:toggle({ name="TEC9 Ammo 50",    flag="ammo_tec9_50",  default=false })
    s_pistol:toggle({ name="M9 Ammo 50",      flag="ammo_m9_50",    default=false })
    s_pistol:toggle({ name="PP19 Ammo 64",    flag="ammo_pp19_64",  default=false })
    s_pistol:toggle({ name="PPSH Ammo 75",    flag="ammo_ppsh_75",  default=false })
    s_pistol:toggle({ name="M1911 Ammo 7",    flag="ammo_m1911_7",  default=false })
    s_pistol:toggle({ name="Maverick Ammo 6", flag="ammo_mav_6",    default=false })
    s_pistol:toggle({ name="Ruger22 Ammo 10", flag="ammo_ruger_10", default=false })
...
-- ============================================================
--  FRAME FUNCTION (called every frame from LUA tab)
-- ============================================================
_G.esp_frame = function()
    local library = _G.esp_lib
    local Chunks  = _G.Chunks

    _G.esp_window:Render()
